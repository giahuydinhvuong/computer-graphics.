...
{	 
	...
	int x = 0;
	int y = 0;
	double P = (double) 40*sin(((double)x+1)/20)-2*y-1;
	//cout<<x<<" "<<y<<" "<<P<<"\n";
	while(x<PI*10) {
		draw2point(xc, yc, x,y, xBound);
		if (P<0) P += (double) 40*sin(((double)x+2)/20)-40*sin(((double)x+1)/20);
    	else {
    		P += (double)40*sin(((double)x+2)/20)-40*sin(((double)x+1)/20)-2;
    		y++;
		}
		x++;
		draw2point(xc, yc, x,y, xBound);
	...
	P = (double) -40*sin(((double)x+1)/20)+2*y-1;
	while(x<PI*30) {
		if (P<0) P += (double) -40*sin(((double)x+2)/20)+40*sin(((double)x+1)/20);
    	else {
    		P += (double)-40*sin(((double)x+2)/20)+40*sin(((double)x+1)/20)-2;
...
int main(int argc, char** argv){
	...
	glutCreateWindow("DEMO THUAT TOAN VE SIN(X) - BRESENHAM");
	initGL();
